...
const StylishInputs = () => {
  ...
  // Remove authentication check - direct access
  ...
  const [description, setDescription] = useState("");       // for item description
const [detailImages, setDetailImages] = useState([]);     // multiple images for slider
const [selectedSizes, setSelectedSizes] = useState([]);
  ...
  ...prev,
  ...validImages.filter(newImg => !prev.some(img => img.uri === newImg.uri))
]);
  }
};
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      Alert.alert("Error", `Failed to fetch items: ${err.response?.data?.error?.message || err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  // ✅ Add item
  const addItem = async () => {
    if (!price || !category || !itemName || !image || detailImages.length === 0) {
    ...
    try {
      ...
      if (!image.base64) {
        Alert.alert("Error", "Image data is missing. Please select a image again.");
        return;
      }
      ...
      console.log("📤 Preparing upload:", {
       ...
       detailImagesBase64: (detailImages || []).map(img => `data:image/jpeg;base64,${img.base64}`), // array
        sizes: selectedSizes,
      });
  ...
  const handleSignOut = async () => {
       setLoading(true);
       try {
                // Direct sign out and navigation
                await clerkSignOut();
                router.replace('/');
  ...
  if (loading) {
    return (
      <View style={[styles.container, { flex: 1, justifyContent: "center", alignItems: "center" }]}>
        ...
        <Text style={{ marginTop: 10, color: "#666" }}>Loading admin panel...</Text>
      </View>
    );
  }
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.card}>
       ...
       />
        ...
        borderRadius: 10,
        borderWidth: 1,
        borderColor: selectedSizes.includes(size) ? "#FFA500" : "#ddd",
        backgroundColor: selectedSizes.includes(size) ? "#FFF4E1" : "#fff",
          }}
          >
         <Text>{size}</Text>
       </TouchableOpacity>
      ...
      <View style={{paddingTop:10,paddingBottom:10,justifyContent:"center",paddingTop:20}}>
       <TouchableOpacity style={{height:40}} onPress={pickDetailImages}>
         <Text style={{backgroundColor:"#007BFF",width:"60%",padding:12,color:"white",borderRadius:10,alignItems:"center",textAlign:"center"}}>Choose Detail Images</Text>
       </TouchableOpacity>
       </View>
       <View>
  ...
  <ScrollView horizontal style={{ marginTop: 10 }}>
    {detailImages.map((img, idx) => (
      <View key={idx} style={{ marginRight: 10, position: 'relative' }}>
        <Image 
          source={{ uri: img.uri }}
          style={{ width: 100, height: 100, borderRadius: 8,marginTop:15 }}
        />
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.itemBox}>
            <Image source={{ uri: item.imageUrl || item.image_url }} style={styles.itemImage} />
...
const styles = StyleSheet.create({
  ...
  sectionHeading: { fontSize: 20, fontWeight: "700", marginBottom: 15, textAlign: "center", color: "#333" },
  signOutBtn: { 
  ...
  card: { backgroundColor: "#fff", padding: 15, borderRadius: 15, marginBottom: 20, shadowColor: "#000", shadowOpacity: 0.1, shadowOffset: { width: 0, height: 2 }, shadowRadius: 6, elevation: 4 },
  input: { borderWidth: 1, borderColor: "#ddd", borderRadius: 10, padding: 10, marginBottom: 12, backgroundColor: "#fafafa" },
  row: { flexDirection: "row", justifyContent: "space-between", marginVertical: 10, paddingTop:20},
  btnPrimary: { backgroundColor: "#007BFF", padding: 12, borderRadius: 10, flex: 0.48, alignItems: "center" },
  btnSuccess: { backgroundColor: "#28a745", padding: 12, borderRadius: 10, flex: 0.48, alignItems: "center" },
  btnText: { color: "#fff", fontWeight: "600" },
  previewBox: { marginTop: 15,  },
  previewImage: { width: 100, height: 100, borderRadius: 12, resizeMode: "cover" },
  listHeading: { fontSize: 20, fontWeight: "700", marginBottom: 15, color: "#444" },
  itemBox: { flexDirection: "row", alignItems: "center", padding: 12, backgroundColor: "#fff", borderRadius: 12, marginBottom: 12, shadowColor: "#000", shadowOpacity: 0.08, shadowOffset: { width: 0, height: 2 }, shadowRadius: 5, elevation: 3 },
  itemImage: { width: 65, height: 65, borderRadius: 8 },
  itemText: { fontSize: 16, fontWeight: "600", color: "#333" },
  subText: { color: "#555" },
  deleteBtn: { backgroundColor: "red", paddingVertical: 8, paddingHorizontal: 12, borderRadius: 8 },
});
